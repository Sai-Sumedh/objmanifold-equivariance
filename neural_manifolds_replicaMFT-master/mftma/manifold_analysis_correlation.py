...
code is borrowed from: https://github.com/schung039/neural_manifolds_replicaMFT/tree/master
and modified, to solve for various package dependency issues
'''
...
def CGmanopt(X, objective_function, A, **kwargs):
    ...
    from pymanopt.function import autograd as aug
    @aug(manifold)
    def cost(X):
        c, _ = objective_function(X, A)
        return c

    problem = Problem(manifold=manifold, cost=cost)#, verbosity=0) #edited to remove verbosity since it is no longer a valid argument
    solver = ConjugateGradient()#logverbosity=0) #removed invalid argument logverbosity
    # Xopt = solver.solve(problem) #solve is not a valid function, so use 'run'
